...
\op{Param} & $(\mathbf{s})$ or $(\mathbf{s}, \text{min}, \text{max})$ & slot, dtype, device? &
  Placeholder with shape $\mathbf{s}$. Slot indexes into \op{Call} src. \\[4pt]
...
\textbf{Op} & \textbf{src} & \textbf{arg} & \textbf{Semantics} \\
\midrule
\op{Call} & $(s_0, s_1, \ldots)$ & --- & Substitute each \op{Param} $k$ with $s_k$. Multiple bodies, void output. \\
\op{Extract} & $(\text{compound},)$ & idx & Extract element. \op{Call}: $\mathrm{src}[\mathrm{idx}]$. \op{After}: $\mathrm{src}[\mathrm{idx}{+}1]$. \\
...
\textbf{Op} & \textbf{src} & \textbf{arg} & \textbf{Semantics} \\
\midrule
\op{Store} & (buf, val) & --- & Write val into buf. buf.shape $=$ val.shape. Output is void with no shape. \\
\bottomrule
\end{tabular}
...
\op{Call} & void & $()$ & \textsc{null} & --- \\
\op{Extract} & $t.\mathrm{dtype}$ & $t.\mathrm{shape}$ & $t.\mathrm{device}$ & $t$ \\
...
\op{Extract} target $t$: $\mathrm{src}[0].\mathrm{src}[\mathrm{idx}]$ for \op{Call}, $\mathrm{src}[0].\mathrm{src}[\mathrm{idx}{+}1]$ for \op{After}.
