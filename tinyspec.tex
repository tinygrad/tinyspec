...
\op{BufferView} & (buf,) & size, dtype, offset &
  Typed access into a buffer. Zero-copy $(\textit{size},)$ slice at offset. \\
...
The result is a reusable graph fragment: the body contains only \op{Param} references, not concrete buffers. At schedule time, the \op{Call} is resolved by substituting each \op{Param}$(k)$ back with its corresponding argument $a_k$.


# new subsection, lowering pipeline

Callify -- Transform the Tensor graph into a single stateless function.

Rangeify -- Determine the kernel split of the function.

Optimize -- Swap and split ranges, determine which are parallel and which are serial

Expander -- Expand the parallel ranges (into shape, not dtype.vec)

Instruction Selection -- Includes WMMA and devectorizer

Linearize -- Toposort and determine the order everything happens

Register/Memory Plan -- Once we have the order, we can reuse non overlapping memory.
