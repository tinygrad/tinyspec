...
The core optimization is \op{Split}: split an axis $n$ by factor $k$ into $(n/k, k)$ --- or $(k, n/k)$ if top --- and assign the new sub-axis a target AxisType.
...
\midrule
\texttt{GLOBAL} & \texttt{g} & --- & --- & GPU global workgroup dimension. \\
\texttt{LOCAL} & \texttt{l} & g, L & inner & Workgroup local dimension (shared memory). \\
\texttt{WARP} & \texttt{w} & \multicolumn{2}{l}{(created by \op{TC})} & Warp-level lanes for tensor cores. \\
\texttt{THREAD} & \texttt{t} & g & outer & CPU thread parallelism. \\
\texttt{LOOP} & \texttt{L} & --- & --- & Generic sequential loop (initial state). \\
\texttt{REDUCE} & \texttt{R} & --- & --- & Reduction axis. \\
\texttt{GROUP\_REDUCE} & \texttt{G} & R & inner/outer & Shared-memory group reduction. \\
\texttt{UPCAST} & \texttt{u} & g, l, L & inner & Register-level vectorization. \\
\texttt{UNROLL} & \texttt{r} & R, G & inner & Fully unrolled loop ($k \le 32$). \\
\bottomrule
\end{tabular}

\medskip
The remaining optimizations are a triple $(\mathrm{op},\;\mathrm{axis},\;\mathrm{arg})$:
...
\op{Padto} & any & multiple $m$ &
  Pad axis to next multiple of $m$ with validity masks. \\
\op{Swap} & g$_i$ & g$_j$ &
  Swap two \texttt{GLOBAL} axes $i \leftrightarrow j$. \\
\op{Nolocals} & --- & --- &
